...
class FSConnectionSpec
  ...
  trait FSConnectionFixture {
    ...
    implicit val _adapter: MarkerLoggingAdapter =
    ...
    val connection = new FSConnection {
      ...
      override implicit protected val materializer: Materializer =
        actorMaterializer
      override implicit protected val adapter: MarkerLoggingAdapter = _adapter

      override def enableDebugLogs: Boolean = true
    }

    def runGraph(
        bidiFlow: BidiFlow[ByteString, FSData, FSCommand, ByteString, _],
        downstreamSource: Source[FSCommand, _],
        upstreamSource: Source[ByteString, _],
        downstreamSink: Sink[ByteString, Future[ByteString]],
        upstreamSink: Sink[FSData, Future[FSData]]
    ): (Future[ByteString], Future[FSData]) = {
      RunnableGraph
        .fromGraph(GraphDSL.create(downstreamSink, upstreamSink)(Keep.both) {
          implicit b => (st, sb) =>
  ...
  "A handler function" should {
    ...
    "handle downstream stream and push FS command to downstream" in new FSConnectionFixture {
      ...
      whenReady(downStream) { fsCommand =>
  ...
  "FS helper functions" should {
    "generate FS playback command" in new FSConnectionFixture {
      val commandResponse = connection.play(
        "/usr/share/freeswitch/sounds/en/us/callie/voicemail/8000/vm-tutorial_change_pin.wav"
      )
    ...
    "generate FS transfer command with eventLock" in new FSConnectionFixture {
      ...
      val commandResponse = connection.transfer(
        "user/1000",
        ApplicationCommandConfig(channelUuid, true, 2, true)
      )
      whenReady(commandResponse) { response =>
        val fSCommand =
          s"sendmsg ${channelUuid}\nEvent-UUID: ${response.command.eventUuid}\ncall-command: execute\nexecute-app-name: transfer\nevent-lock: true\nloops: 2\nasync: true\ncontent-type: text/plain\ncontent-length: 9\n\nuser/1000\n"
        response.command.toString shouldBe fSCommand
      }
    }
    ...
    "generate FS filter command" in new FSConnectionFixture {
      val commandResponse = connection.filter(
        Map(EventNames.ChannelExecute -> HeaderNames.eventName)
      )
      whenReady(commandResponse) { response =>
    ...
    "generate FS deleteFilter command" in new FSConnectionFixture {
      val commandResponse = connection.deleteFilter(
        Map(EventNames.Heartbeat -> HeaderNames.eventName)
      )
      whenReady(commandResponse) { response =>
    ...
    "generate FS bridge command to dial multiple contacts all at once " in new FSConnectionFixture {
      val commandResponse =
        connection.bridge(List("user/5000", "user/5001"), AllAtOnce)
      whenReady(commandResponse) { response =>
        val fSCommand =
    ...
    "generate FS bridge command to dial multiple contacts one at a time" in new FSConnectionFixture {
      val commandResponse =
        connection.bridge(List("user/5000", "user/5001"), OneAtATime)
      whenReady(commandResponse) { response =>
        val fSCommand =
          s"sendmsg \nEvent-UUID: ${response.command.eventUuid}\ncall-command: execute\nexecute-app-name: bridge\ncontent-type: text/plain\ncontent-length: 19\n\nuser/5000|user/5001\n"
        response.command.toString shouldBe fSCommand
    ...
    "generate FS record command" in new FSConnectionFixture {
      val commandResponse = connection.record(
        "/tmp/record.mp3",
        Duration(5, SECONDS),
        Duration(3, SECONDS)
      )
    ...
    "generate FS sendDtmf command with duration" in new FSConnectionFixture {
      val commandResponse = connection.sendDtmf(
        "0123456789ABCD*#@100",
        Some(Duration(400, MILLISECONDS))
      )
      whenReady(commandResponse) { response =>
        val fSCommand =
    ...
    "generate FS log command" in new FSConnectionFixture {
      val commandResponse =
        connection.log("DIALING Extension DialURI [${sip_uri_to_dial}]")
      whenReady(commandResponse) { response =>
