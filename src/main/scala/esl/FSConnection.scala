...
abstract class FSConnection extends StrictLogging {
  ...
  def transferByReferContext: Option[String]
      ...
      .statefulMapConcat(() => {
        ...
        def filterFSMessages(fSData: FSData): FSData = {
          ...
          if (messagesWithDifferentId.nonEmpty) {
            ...
            val relatedChannels = transferByReferContext match {
              case Some(transferHandler) => {
                messagesWithDifferentId.flatMap({
                  case eventMessage: EventMessage
                      if eventMessage.eventName
                        .exists(_.name == EventNames.ChannelOriginate.name) &&
                        eventMessage
                          .getHeader(HeaderNames.CallerContext)
                          .contains(transferHandler) && !eventMessage.uuid
  ...
  private def handleFSEventMessage(eventMessage: EventMessage): EventMessage = {
    ...
    transferByReferContext.foreach({ transferContext =>
      eventMessage.eventName.foreach({
        ...
        case EventNames.ChannelOriginate
            if eventMessage
              .getHeader(HeaderNames.CallerContext)
              .contains(transferContext) && eventMessage.headers
    ...
    eventMessage.applicationUuid match {
      case Some(appId) => {
        (eventMap.get(appId), transferByReferContext) match {
          case (Some(commandToQueue), transferByReferContext) => {
            if (eventMessage.eventName.contains(EventNames.ChannelExecute)) {
              ...
              (
                ...
                case (bridge: Bridge, Some(referedBy), Some(transferContext))
                    ...
                    case (
                      ...
                      silentCallOriginators = silentCallOriginators.map({
                        case s @ SilentTransfers(
                          ...
                          adapter.info(
                            logMarker,
                            s"""handleFSEventMessage ChannelExecuteComplete treated  as transfer (step 2/4) (transferContext :$transferContext)
          ...
          case (_, Some(transferContext))
            ...
            (
                  ...
                  case SilentTransfers(originator, Some(from), Some(to), _, _)
                    ...
                    eventMap.collectFirst({
                      case (
                        ...
                        adapter.info(
                          logMarker,
                          s"""handleFSEventMessage for ChannelExecuteComplete treated as transfer (step 4/4) (transferContext:$transferContext)
              ...
              case _ => {
                adapter.warning(
                  logMarker,
                  s"""handleFSEventMessage for app id $appId Unable to find Command in look up (2) (headers not as expected for silent transfer) (transferByReferContext : $transferContext)
                     |>> TYPE
                     |EVENT ${eventMessage.eventName}
                     |>> HEADERS
                     |${eventMessage.headers
          ...
          case _ => {
            adapter.warning(
              logMarker,
              s"""handleFSEventMessage for app id $appId Unable to find Command in look up (transferByReferContext : $transferByReferContext)
                 |>> TYPE
                 |EVENT ${eventMessage.eventName}
                 |>> HEADERS
                 |${eventMessage.headers
